...
const asyncLessons = require('./data/jsPart37Async');
const modulesLessons = require('./data/jsPart38Modules');
...
const newChapters = [
  { id: 'js-chap-async', title: 'JS Asynchronous', order: 37 },
  { id: 'js-chap-modules', title: 'JS Modules', order: 38 }
];

async function seedJavaScriptPart15() {
  ...
  console.log('🚀 Memulai proses seeding 18 materi baru JS (Asynchronous & Modules)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/257] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 15 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 15:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart15().catch(console.error);
