...
const phpPart20RefException = require('./data/phpPart20RefException');

async function seedPhpPart20() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart20RefException.length} materi Exception Bab 10 (PHP Reference Exception Methods 166-173)...`);

  for (const item of phpPart20RefException) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH METHOD EXCEPTION REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart20().catch(console.error);
