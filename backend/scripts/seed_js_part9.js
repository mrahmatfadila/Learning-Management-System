...
const versionsLessons = require('./data/jsPart25Versions');
...
const newChapters = [
  { id: 'js-chap-versions', title: 'JS Versions', order: 25 }
];

async function seedJavaScriptPart9() {
  ...
  console.log('🚀 Memulai proses seeding 17 materi baru JS (Versions & History)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/153] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 9 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 9:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart9().catch(console.error);
