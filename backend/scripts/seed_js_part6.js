...
const regexpLessons = require('./data/jsPart18RegExp');
const datatypesLessons = require('./data/jsPart19DataTypes');
...
const newChapters = [
  { id: 'js-chap-regexp', title: 'JS RegExp', order: 18 },
  { id: 'js-chap-datatypes', title: 'JS Data Types', order: 19 }
];

async function seedJavaScriptPart6() {
  ...
  console.log('🚀 Memulai proses seeding 22 materi baru JS (RegExp, Data Types)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/112] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 6 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 6:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart6().catch(console.error);
