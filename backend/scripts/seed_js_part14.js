...
const objectsAdvLessons = require('./data/jsPart35ObjectsAdv');
const classesLessons = require('./data/jsPart36Classes');
...
const newChapters = [
  { id: 'js-chap-objadv', title: 'JS Objects', order: 35 },
  { id: 'js-chap-classes', title: 'JS Classes', order: 36 }
];

async function seedJavaScriptPart14() {
  ...
  console.log('🚀 Memulai proses seeding 12 materi baru JS (Objects Advanced & Classes)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/239] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 14 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 14:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart14().catch(console.error);
