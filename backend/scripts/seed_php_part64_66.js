...
const phpPart64RefVar1 = require('./data/phpPart64RefVar1');
const phpPart65RefVar2 = require('./data/phpPart65RefVar2');
const phpPart66RefVar3 = require('./data/phpPart66RefVar3');

async function seedPhpPart64_66() {
  ...
  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Variable Handling Reference Bab 10 (PHP Reference Variable 696-723)...`);
  ...
  console.log(`\n🎉 SELURUH VARIABLE HANDLING REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart64_66().catch(console.error);
