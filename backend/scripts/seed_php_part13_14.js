...
const phpPart13RefArrays2 = require('./data/phpPart13RefArrays2');
const phpPart14RefArrays3 = require('./data/phpPart14RefArrays3');

async function seedPhpPart13_14() {
  ...
  const allLessons = [...phpPart13RefArrays2, ...phpPart14RefArrays3];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi lanjutan Bab 10 (PHP Reference Array Functions 28-79)...`);
  ...
  console.log(`\n🎉 SELURUH FUNGSI ARRAY REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart13_14().catch(console.error);
