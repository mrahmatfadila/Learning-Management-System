...
const loopsLessons = require('./data/jsPart5Loops');
const stringsLessons = require('./data/jsPart6Strings');
const numbersLessons = require('./data/jsPart7Numbers');

const allNewJsLessons = [...loopsLessons, ...stringsLessons, ...numbersLessons];

const newChapters = [
  { id: 'js-chap-loops', title: 'JS Loops', order: 5 },
  { id: 'js-chap-strings', title: 'JS Strings', order: 6 },
  { id: 'js-chap-numbers', title: 'JS Numbers', order: 7 }
];

async function seedJavaScriptPart2() {
  ...
  console.log('🚀 Memulai proses seeding 17 materi baru JS (Loops, Strings, Numbers)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/39] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 2 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 2:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart2().catch(console.error);
