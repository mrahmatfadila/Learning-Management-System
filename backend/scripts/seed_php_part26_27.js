...
const phpPart26RefFtp1 = require('./data/phpPart26RefFtp1');
const phpPart27RefFtp2 = require('./data/phpPart27RefFtp2');

async function seedPhpPart26_27() {
  ...
  const allLessons = [...phpPart26RefFtp1, ...phpPart27RefFtp2];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi FTP Functions Bab 10 (PHP Reference FTP 262-296)...`);
  ...
  console.log(`\n🎉 SELURUH FTP REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart26_27().catch(console.error);
