...
const windowApiLessons = require('./data/jsPart29WindowApi');
const fetchApiLessons = require('./data/jsPart30FetchApi');
const jsonLessons = require('./data/jsPart31Json');
const webApiLessons = require('./data/jsPart32WebApi');
...
const newChapters = [
  { id: 'js-chap-windowapi', title: 'JS Window API', order: 29 },
  { id: 'js-chap-fetchapi', title: 'JS Fetch API', order: 30 },
  { id: 'js-chap-json', title: 'JS JSON', order: 31 },
  { id: 'js-chap-webapi', title: 'JS Web API', order: 32 }
];

async function seedJavaScriptPart11() {
  ...
  console.log('🚀 Memulai proses seeding 22 materi baru JS (Window, Fetch, JSON, Web API)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/195] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 11 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 11:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart11().catch(console.error);
