...
const phpPart21RefFilesystem1 = require('./data/phpPart21RefFilesystem1');
const phpPart22RefFilesystem2 = require('./data/phpPart22RefFilesystem2');

async function seedPhpPart21_22() {
  ...
  const allLessons = [...phpPart21RefFilesystem1, ...phpPart22RefFilesystem2];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Filesystem Bab 10 (PHP Reference Filesystem Functions 174-220)...`);
  ...
  console.log(`\n🎉 SELURUH FUNGSI FILESYSTEM REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart21_22().catch(console.error);
