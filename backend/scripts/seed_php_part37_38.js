...
const phpPart37RefMath3 = require('./data/phpPart37RefMath3');
const phpPart38RefMath4 = require('./data/phpPart38RefMath4');

async function seedPhpPart37_38() {
  ...
  const allLessons = [...phpPart37RefMath3, ...phpPart38RefMath4];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Math Reference Lanjutan Bab 10 (PHP Reference Math 396-419)...`);
  ...
  console.log(`\n🎉 SELURUH MATH REFERENCE BAB 10 SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart37_38().catch(console.error);
