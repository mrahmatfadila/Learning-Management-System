...
const phpPart41RefMysqli1 = require('./data/phpPart41RefMysqli1');
const phpPart42RefMysqli2 = require('./data/phpPart42RefMysqli2');

async function seedPhpPart41_42() {
  ...
  const allLessons = [...phpPart41RefMysqli1, ...phpPart42RefMysqli2];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi MySQLi Reference Bab 10 (PHP Reference MySQLi 445-462)...`);
  ...
  console.log(`\n🎉 SELURUH MYSQLI REFERENCE PART 1 & 2 SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart41_42().catch(console.error);
