...
const phpPart35RefMath1 = require('./data/phpPart35RefMath1');
const phpPart36RefMath2 = require('./data/phpPart36RefMath2');

async function seedPhpPart35_36() {
  ...
  const allLessons = [...phpPart35RefMath1, ...phpPart36RefMath2];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Math Reference Bab 10 (PHP Reference Math 371-395)...`);
  ...
  console.log(`\n🎉 SELURUH MATH REFERENCE PART 1 & 2 SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart35_36().catch(console.error);
