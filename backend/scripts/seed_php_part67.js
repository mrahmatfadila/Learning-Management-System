...
const phpPart67RefVar4 = require('./data/phpPart67RefVar4');

async function seedPhpPart67() {
  ...
  const allLessons = phpPart67RefVar4;

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Variable Handling Reference Part 4 Bab 10 (PHP Reference Variable 724-730)...`);
  ...
  console.log(`\n🎉 SELURUH VARIABLE HANDLING REFERENCE SELESAI LENGKAP! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart67().catch(console.error);
