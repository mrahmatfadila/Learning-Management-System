...
const errorsLessons = require('./data/jsPart20Errors');
const debuggingLessons = require('./data/jsPart21Debugging');
const styleGuideLessons = require('./data/jsPart22StyleGuide');
...
const newChapters = [
  { id: 'js-chap-errors', title: 'JS Errors', order: 20 },
  { id: 'js-chap-debugging', title: 'JS Debugging', order: 21 },
  { id: 'js-chap-styleguide', title: 'JS Style Guide', order: 22 }
];

async function seedJavaScriptPart7() {
  ...
  console.log('🚀 Memulai proses seeding 14 materi baru JS (Errors, Debugging, Style Guide)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/126] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 7 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 7:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart7().catch(console.error);
