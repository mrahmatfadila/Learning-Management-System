...
const phpPart15RefCalendar = require('./data/phpPart15RefCalendar');

async function seedPhpPart15() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart15RefCalendar.length} materi Kalender Bab 10 (PHP Reference Calendar Functions 80-97)...`);

  for (const item of phpPart15RefCalendar) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH FUNGSI CALENDAR REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart15().catch(console.error);
