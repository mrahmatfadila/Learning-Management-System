...
const domLessons = require('./data/jsPart26Dom');
const eventsLessons = require('./data/jsPart27Events');
const htmlFirstLessons = require('./data/jsPart28HtmlFirst');
...
const newChapters = [
  { id: 'js-chap-htmldom', title: 'JS HTML DOM', order: 26 },
  { id: 'js-chap-htmlevents', title: 'JS HTML Events', order: 27 },
  { id: 'js-chap-htmlfirst', title: 'JS HTML First', order: 28 }
];

async function seedJavaScriptPart10() {
  ...
  console.log('🚀 Memulai proses seeding 20 materi baru JS (DOM, Events, HTML First)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/173] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 10 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 10:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart10().catch(console.error);
