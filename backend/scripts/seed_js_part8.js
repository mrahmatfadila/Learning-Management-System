...
const referenceLessons = require('./data/jsPart23Reference');
const projectsLessons = require('./data/jsPart24Projects');
...
const newChapters = [
  { id: 'js-chap-reference', title: 'JS Reference', order: 23 },
  { id: 'js-chap-projects', title: 'JS Projects', order: 24 }
];

async function seedJavaScriptPart8() {
  ...
  console.log('🚀 Memulai proses seeding 10 materi baru JS (Reference & Projects)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/136] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 8 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 8:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart8().catch(console.error);
