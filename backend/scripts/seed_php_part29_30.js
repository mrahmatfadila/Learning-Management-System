...
const phpPart29RefKeywords1 = require('./data/phpPart29RefKeywords1');
const phpPart30RefKeywords2 = require('./data/phpPart30RefKeywords2');

async function seedPhpPart29_30() {
  ...
  const allLessons = [...phpPart29RefKeywords1, ...phpPart30RefKeywords2];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Keywords Bab 10 (PHP Reference Keywords 300-326)...`);
  ...
  console.log(`\n🎉 SELURUH KEYWORDS PART 1 & 2 SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart29_30().catch(console.error);
