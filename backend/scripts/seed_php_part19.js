...
const phpPart19RefError = require('./data/phpPart19RefError');

async function seedPhpPart19() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart19RefError.length} materi Error & Debugging Bab 10 (PHP Reference Error Functions 156-165)...`);

  for (const item of phpPart19RefError) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH FUNGSI ERROR & DEBUGGING REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart19().catch(console.error);
