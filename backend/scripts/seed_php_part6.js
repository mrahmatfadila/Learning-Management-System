...
const phpPart6Oop = require('./data/phpPart6Oop');

async function seedPhpPart6() {
  ...
  console.log('🚀 Memulai pengisian Bab 4 (PHP OOP) ke database lms_content_db & lms_edutech_db...');

  // 1. Pastikan Chapter 4: PHP OOP ada
  await pTarget.query(`
  ...
  // 2. Masukkan 14 Materi Bab 4 (PHP OOP)
  console.log(`\n📚 Memproses ${phpPart6Oop.length} materi Bab 4 (PHP OOP)...`);

  for (const item of phpPart6Oop) {
    const fullContent = {
    ...
    console.log(`  ✅ [Bab 4 / Materi ${item.order}] ${item.title} (${item.id}) -> Sukses tersimpan`);
  }
  ...
  console.log(`\n🎉 BAB 4 SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart6().catch(console.error);
