...
const phpPart23RefFilesystem3 = require('./data/phpPart23RefFilesystem3');
const phpPart24RefFilesystem4 = require('./data/phpPart24RefFilesystem4');

async function seedPhpPart23_24() {
  ...
  const allLessons = [...phpPart23RefFilesystem3, ...phpPart24RefFilesystem4];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Filesystem Lanjutan Bab 10 (PHP Reference Filesystem Functions 221-254)...`);
  ...
  console.log(`\n🎉 SELURUH FUNGSI FILESYSTEM LANJUTAN SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart23_24().catch(console.error);
