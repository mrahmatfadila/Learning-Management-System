...
const scopeLessons = require('./data/jsPart11Scope');
const datesLessons = require('./data/jsPart12Dates');
const arraysLessons = require('./data/jsPart13Arrays');

const allNewJsLessons = [...scopeLessons, ...datesLessons, ...arraysLessons];

const newChapters = [
  { id: 'js-chap-scope', title: 'JS Scope', order: 11 },
  { id: 'js-chap-dates', title: 'JS Dates', order: 12 },
  { id: 'js-chap-arrays', title: 'JS Arrays', order: 13 }
];

async function seedJavaScriptPart4() {
  ...
  console.log('🚀 Memulai proses seeding 18 materi baru JS (Scope, Dates, Arrays)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/74] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 4 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 4:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart4().catch(console.error);
