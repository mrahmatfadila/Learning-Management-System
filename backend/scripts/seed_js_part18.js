...
const examplesLessons = require('./data/jsPart46Examples');

const newChapters = [
  { id: 'js-chap-examples', title: 'JS Examples', order: 46 }
];

async function seedJavaScriptPart18() {
  ...
  console.log('🚀 Memulai proses seeding 14 materi baru JS (JS Examples Capstone Suite)...');
  ...
  try {
    ...
    for (const item of examplesLessons) {
      ...
      console.log(`    📄 [${item.order}/306] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 18 SUKSES! Total: ${examplesLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 18:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart18().catch(console.error);
