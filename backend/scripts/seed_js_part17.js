...
const ajaxLessons = require('./data/jsPart43Ajax');
const jqueryLessons = require('./data/jsPart44Jquery');
const jsonpLessons = require('./data/jsPart45Jsonp');
...
const newChapters = [
  { id: 'js-chap-ajax', title: 'JS AJAX', order: 43 },
  { id: 'js-chap-jquery', title: 'JS jQuery', order: 44 },
  { id: 'js-chap-jsonp', title: 'JS JSONP', order: 45 }
];

async function seedJavaScriptPart17() {
  ...
  console.log('🚀 Memulai proses seeding 15 materi baru JS (AJAX, jQuery, JSONP)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/292] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 17 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 17:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart17().catch(console.error);
