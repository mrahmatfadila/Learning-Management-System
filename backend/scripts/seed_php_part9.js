...
const phpPart9Ajax = require('./data/phpPart9Ajax');

async function seedPhpPart9() {
  ...
  console.log('🚀 Memulai pengisian Bab 7 (PHP - AJAX) ke database lms_content_db & lms_edutech_db...');

  // 1. Pastikan Chapter 7: PHP - AJAX ada
  await pTarget.query(`
  ...
  // 2. Masukkan 6 Materi Bab 7 (PHP - AJAX)
  console.log(`\n📚 Memproses ${phpPart9Ajax.length} materi Bab 7 (PHP - AJAX)...`);

  for (const item of phpPart9Ajax) {
    const fullContent = {
    ...
    console.log(`  ✅ [Bab 7 / Materi ${item.order}] ${item.title} (${item.id}) -> Sukses tersimpan`);
  }
  ...
  console.log(`\n🎉 BAB 7 SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart9().catch(console.error);
