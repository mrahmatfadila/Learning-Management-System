...
const phpPart39RefMisc1 = require('./data/phpPart39RefMisc1');
const phpPart40RefMisc2 = require('./data/phpPart40RefMisc2');

async function seedPhpPart39_40() {
  ...
  const allLessons = [...phpPart39RefMisc1, ...phpPart40RefMisc2];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Misc Reference Bab 10 (PHP Reference Misc 420-444)...`);
  ...
  console.log(`\n🎉 SELURUH MISC REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart39_40().catch(console.error);
