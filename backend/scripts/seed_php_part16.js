...
const phpPart16RefDate = require('./data/phpPart16RefDate');

async function seedPhpPart16() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart16RefDate.length} materi Date & Time Bab 10 (PHP Reference Date Functions 98-118)...`);

  for (const item of phpPart16RefDate) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH FUNGSI DATE REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart16().catch(console.error);
