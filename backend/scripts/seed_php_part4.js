...
const phpPart4Forms = require('./data/phpPart4Forms');

async function seedPhpPart4() {
  ...
  console.log('🚀 Memulai pengisian Bab 2 (PHP Forms) ke database lms_content_db & lms_edutech_db...');

  // 1. Pastikan Chapter 2: PHP Forms ada
  await pTarget.query(`
  ...
  // 2. Masukkan 5 Materi Bab 2 (PHP Forms)
  console.log(`\n📚 Memproses ${phpPart4Forms.length} materi Bab 2 (PHP Forms)...`);

  for (const item of phpPart4Forms) {
    const fullContent = {
    ...
    console.log(`  ✅ [Bab 2 / Materi ${item.order}] ${item.title} (${item.id}) -> Sukses tersimpan`);
  }
  ...
  console.log(`\n🎉 BAB 2 SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart4().catch(console.error);
