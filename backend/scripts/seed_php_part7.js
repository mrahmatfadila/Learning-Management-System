...
const phpPart7Mysql = require('./data/phpPart7Mysql');

async function seedPhpPart7() {
  ...
  console.log('🚀 Memulai pengisian Bab 5 (MySQL Database) ke database lms_content_db & lms_edutech_db...');

  // 1. Pastikan Chapter 5: MySQL Database ada
  await pTarget.query(`
  ...
  // 2. Masukkan 14 Materi Bab 5 (MySQL Database)
  console.log(`\n📚 Memproses ${phpPart7Mysql.length} materi Bab 5 (MySQL Database)...`);

  for (const item of phpPart7Mysql) {
    const fullContent = {
    ...
    console.log(`  ✅ [Bab 5 / Materi ${item.order}] ${item.title} (${item.id}) -> Sukses tersimpan`);
  }
  ...
  console.log(`\n🎉 BAB 5 SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart7().catch(console.error);
