...
const phpPart28RefJson = require('./data/phpPart28RefJson');

async function seedPhpPart28() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart28RefJson.length} materi JSON Reference Bab 10 (PHP Reference JSON 297-299)...`);

  for (const item of phpPart28RefJson) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH JSON REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart28().catch(console.error);
