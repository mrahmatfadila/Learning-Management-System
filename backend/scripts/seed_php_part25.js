...
const phpPart25RefFilter = require('./data/phpPart25RefFilter');

async function seedPhpPart25() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart25RefFilter.length} materi Filter Functions Bab 10 (PHP Reference Filter 255-261)...`);

  for (const item of phpPart25RefFilter) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH FILTER REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart25().catch(console.error);
