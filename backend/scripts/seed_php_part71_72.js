...
const phpPart71RefZip = require('./data/phpPart71RefZip');
const phpPart72RefTimezones = require('./data/phpPart72RefTimezones');

async function seedPhpPart71_72() {
  ...
  const allLessons = [
    ...phpPart71RefZip,
    ...phpPart72RefTimezones
  ];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Zip & Timezones Reference Bab 10 (PHP Reference 755-771)...`);
  ...
  console.log(`\n🎉 SELURUH ZIP & TIMEZONES REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart71_72().catch(console.error);
