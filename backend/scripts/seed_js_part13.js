...
const funcAdvLessons = require('./data/jsPart34FunctionsAdv');
...
const newChapters = [
  { id: 'js-chap-funcadv', title: 'JS Functions Advanced', order: 34 }
];

async function seedJavaScriptPart13() {
  ...
  console.log('🚀 Memulai proses seeding 11 materi baru JS (Functions Advanced Suite)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/227] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 13 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 13:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart13().catch(console.error);
