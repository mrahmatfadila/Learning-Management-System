...
const phpPart34RefMail = require('./data/phpPart34RefMail');

async function seedPhpPart34() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart34RefMail.length} materi Mail Reference Bab 10 (PHP Reference Mail 369-370)...`);

  for (const item of phpPart34RefMail) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH MAIL REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart34().catch(console.error);
