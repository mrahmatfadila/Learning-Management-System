...
const phpPart47RefNetwork1 = require('./data/phpPart47RefNetwork1');
const phpPart48RefNetwork2 = require('./data/phpPart48RefNetwork2');
const phpPart49RefNetwork3 = require('./data/phpPart49RefNetwork3');

async function seedPhpPart47_49() {
  ...
  const allLessons = [...phpPart47RefNetwork1, ...phpPart48RefNetwork2, ...phpPart49RefNetwork3];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Network Reference Bab 10 (PHP Reference Network 512-544)...`);
  ...
  console.log(`\n🎉 SELURUH NETWORK REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart47_49().catch(console.error);
