...
const phpPart17RefDateTimezone = require('./data/phpPart17RefDateTimezone');

async function seedPhpPart17() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart17RefDateTimezone.length} materi Date & Timezone Bab 10 (PHP Reference Date & Timezone Functions 119-146)...`);

  for (const item of phpPart17RefDateTimezone) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH FUNGSI DATE & TIMEZONE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart17().catch(console.error);
