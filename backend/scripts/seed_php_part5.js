...
const phpPart5Advanced = require('./data/phpPart5Advanced');

async function seedPhpPart5() {
  ...
  console.log('🚀 Memulai pengisian Bab 3 (PHP Advanced) ke database lms_content_db & lms_edutech_db...');

  // 1. Pastikan Chapter 3: PHP Advanced ada
  await pTarget.query(`
  ...
  // 2. Masukkan 13 Materi Bab 3 (PHP Advanced)
  console.log(`\n📚 Memproses ${phpPart5Advanced.length} materi Bab 3 (PHP Advanced)...`);

  for (const item of phpPart5Advanced) {
    const fullContent = {
    ...
    console.log(`  ✅ [Bab 3 / Materi ${item.order}] ${item.title} (${item.id}) -> Sukses tersimpan`);
  }
  ...
  console.log(`\n🎉 BAB 3 SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart5().catch(console.error);
