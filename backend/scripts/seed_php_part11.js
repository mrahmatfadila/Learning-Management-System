...
const phpPart11Examples = require('./data/phpPart11Examples');

async function seedPhpPart11() {
  ...
  console.log('🚀 Memulai pengisian Bab 9 (PHP Examples) ke database lms_content_db & lms_edutech_db...');

  // 1. Pastikan Chapter 9: PHP Examples ada
  await pTarget.query(`
  ...
  // 2. Masukkan 8 Materi Bab 9 (PHP Examples)
  console.log(`\n📚 Memproses ${phpPart11Examples.length} materi Bab 9 (PHP Examples)...`);

  for (const item of phpPart11Examples) {
    const fullContent = {
    ...
    console.log(`  ✅ [Bab 9 / Materi ${item.order}] ${item.title} (${item.id}) -> Sukses tersimpan`);
  }
  ...
  console.log(`\n🎉 KURIKULUM MODUL PHP MENCAPAI 122 MATERI LENGKAP! Total materi PHP di lms_content_db: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart11().catch(console.error);
