...
const functionsLessons = require('./data/jsPart8Functions');
const timersLessons = require('./data/jsPart9Timers');
const objectsLessons = require('./data/jsPart10Objects');

const allNewJsLessons = [...functionsLessons, ...timersLessons, ...objectsLessons];

const newChapters = [
  { id: 'js-chap-functions', title: 'JS Functions', order: 8 },
  { id: 'js-chap-timers', title: 'JS Timers', order: 9 },
  { id: 'js-chap-objects', title: 'JS Objects', order: 10 }
];

async function seedJavaScriptPart3() {
  ...
  console.log('🚀 Memulai proses seeding 17 materi baru JS (Functions, Timers, Objects)...');
  ...
  try {
    ...
    for (const item of allNewJsLessons) {
      ...
      console.log(`    📄 [${item.order}/56] ${item.title} -> OK`);
    }
    ...
    console.log(`\n🎉 SEEDING PART 3 SUKSES! Total: ${allNewJsLessons.length} materi baru berhasil dimasukkan ke database.`);
  } catch (err) {
    console.error('❌ Error saat seeding part 3:', err);
    throw err;
  } finally {
    await pContent.end();
    await pEdutech.end();
...
seedJavaScriptPart3().catch(console.error);
