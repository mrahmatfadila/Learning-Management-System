...
const phpPart50RefOutput1 = require('./data/phpPart50RefOutput1');
const phpPart51RefOutput2 = require('./data/phpPart51RefOutput2');

async function seedPhpPart50_51() {
  ...
  const allLessons = [...phpPart50RefOutput1, ...phpPart51RefOutput2];

  console.log(`🚀 Memulai pengisian ${allLessons.length} materi Output Control Reference Bab 10 (PHP Reference Output 545-560)...`);
  ...
  console.log(`\n🎉 SELURUH OUTPUT CONTROL REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart50_51().catch(console.error);
