...
const phpPart52RefRegex = require('./data/phpPart52RefRegex');

async function seedPhpPart52() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart52RefRegex.length} materi RegEx PCRE Reference Bab 10 (PHP Reference RegEx 561-570)...`);

  for (const item of phpPart52RefRegex) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH REGEX PCRE REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart52().catch(console.error);
