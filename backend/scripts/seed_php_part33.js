...
const phpPart33RefLibxml = require('./data/phpPart33RefLibxml');

async function seedPhpPart33() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart33RefLibxml.length} materi Libxml Reference Bab 10 (PHP Reference Libxml 362-368)...`);

  for (const item of phpPart33RefLibxml) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH LIBXML REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart33().catch(console.error);
