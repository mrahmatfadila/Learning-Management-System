...
const phpPart46RefMysqli6 = require('./data/phpPart46RefMysqli6');

async function seedPhpPart46() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart46RefMysqli6.length} materi MySQLi Reference Part 6 Bab 10 (PHP Reference MySQLi 503-511)...`);

  for (const item of phpPart46RefMysqli6) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH MYSQLI REFERENCE SELESAI LENGKAP! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart46().catch(console.error);
