...
const phpPart18RefDirectory = require('./data/phpPart18RefDirectory');

async function seedPhpPart18() {
  ...
  console.log(`🚀 Memulai pengisian ${phpPart18RefDirectory.length} materi Direktori Bab 10 (PHP Reference Directory Functions 147-155)...`);

  for (const item of phpPart18RefDirectory) {
    const fullContent = {
  ...
  console.log(`\n🎉 SELURUH FUNGSI DIRECTORY REFERENCE SELESAI! Total materi PHP di lms_content_db sekarang: ${resCount.rows[0].count} materi (${chapCount.rows[0].count} Bab).`);

  await pTarget.end();
  await pSource.end();
}

seedPhpPart18().catch(console.error);
